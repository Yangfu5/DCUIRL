...
class CTMR(nn.Module):
    ...
    def forward(self, obs, mtm=False, ema=False, detach=False):
        ...
        x = obs + position
        x = x.transpose(0, 1)
        length_attn = len(self.attn_layers)
        for i in range(length_attn):
...
class CtmrSacAgent(object):
    ...
    def update_cpc_irl(self, cpc_kwargs, L, step):
        obs_anchor, obs_pos = cpc_kwargs["obs_anchor"], cpc_kwargs["obs_pos"]
        # 正常使用encoder -> 50
        z_a = self.CTMR.encode(obs_anchor)
        # ema 表示使用target encoder，不参与更新
        z_pos = self.CTMR.encode(obs_pos, ema=True)
    ...
    def update(self, replay_buffer, is_not_update_cpc, L, step, irl=None):
        if self.encoder_type == 'pixel':
        ...
        if irl is not None:
            with torch.no_grad():
                reward = irl.get_reward_s_a(obs, action)
        ...
        if not is_not_update_cpc and step % self.cpc_update_freq == 0 and self.encoder_type == 'pixel':
